...
try:
    ...
    st.error(f"缺少 API 密鑰: {e}. 請在 Streamlit Cloud 的 Secrets 中設置 ALPHA_VANTAGE_API_KEY 和 POLYGON_API_KEY")
    st.stop()
...
def fetch_price_data(symbol, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            data = response.json()
            fetch_time_alpha = dt.datetime.now()
            
            if "Time Series (Daily)" not in data:
                raise ValueError(f"Alpha Vantage: No valid data for {symbol}: {data.get('Note', 'Unknown error')}")
            ...
            df = df[["date", "close", "volume", "high", "low"]].sort_values("date")
            df = df[(df["date"].dt.date >= start_date) & (df["date"].dt.date <= end_date)]
            
            if df.empty:
                raise ValueError(f"Alpha Vantage: No data within date range for {symbol}")
            
            logging.info(f"Alpha Vantage: Data fetched for {symbol} at {fetch_time_alpha}")
            return df, fetch_time_alpha, "Alpha Vantage"
        except Exception as e:
            logging.error(f"Alpha Vantage fetch_price_data({symbol}) failed on attempt {attempt+1}: {e}")
    ...
    for attempt in range(retries):
        try:
            ...
            response = requests.get(url)
            data = response.json()
            fetch_time_polygon = dt.datetime.now()
            
            if "results" not in data:
                raise ValueError(f"Polygon.io: No valid data for {symbol}: {data.get('status', 'Unknown error')}")
            ...
            if df.empty:
                raise ValueError(f"Polygon.io: No data within date range for {symbol}")
            
            logging.info(f"Polygon.io: Data fetched for {symbol} at {fetch_time_polygon}")
            return df, fetch_time_polygon, "Polygon.io"
        except Exception as e:
            logging.error(f"Polygon.io fetch_price_data({symbol}) failed on attempt {attempt+1}: {e}")
            if attempt == retries - 1:
                break
    ...
    for attempt in range(retries):
        try:
            ...
            fetch_time_yf = dt.datetime.now()
            
            if df.empty or df["Close"].isna().all():
                raise ValueError(f"yfinance: No valid data for {symbol}")
            ...
            return df, fetch_time_yf, "yfinance"
        except Exception as e:
            logging.error(f"yfinance fetch_price_data({symbol}) failed on attempt {attempt+1}: {e}")
            if attempt == retries - 1:
                st.error(f"所有數據來源均失敗: {symbol}")
...
# 選擇最新數據
def select_best_data(symbol):
    results = []
    
    # 嘗試所有數據來源
    for source in [("Alpha Vantage", fetch_price_data), ("Polygon.io", fetch_price_data), ("yfinance", fetch_price_data)]:
        df, fetch_time, src = source[1](symbol)
    ...
    # 按最新日期排序，若日期相同則按數據量排序
    results.sort(key=lambda x: (x["latest_date"], x["data_length"]), reverse=True)
    best_result = results[0]
    
    logging.info(f"Selected {best_result['source']} for {symbol}: latest_date={best_result['latest_date']}, fetch_time={best_result['fetch_time']}")
    return best_result["df"], best_result["fetch_time"], best_result["source"]
...
def analyze_single_stock(symbol, sector=""):
    try:
        df, fetch_time, source = select_best_data(symbol)
        ...
        try:
            ...
            if nasdaq_df is not None:
                ...
                df_prophet["nasdaq_close"] = df_prophet["nasdaq_close"].fillna(method="ffill")
                df_prophet["nasdaq_volume"] = df_prophet["nasdaq_volume"].fillna(method="ffill")
            ...
            if nasdaq_df is not None:
                ...
                future["nasdaq_close"] = future["nasdaq_close"].fillna(method="ffill")
                future["nasdaq_volume"] = future["nasdaq_volume"].fillna(method="ffill")
        ...
        try:
            ...
            if len(df_xgb) < 100:
                ...
                if nasdaq_df is not None:
                    ...
                    df_xgb["nasdaq_close"] = df_xgb["nasdaq_close"].fillna(method="ffill")
